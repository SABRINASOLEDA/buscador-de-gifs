...
botonbuscar.addEventListener("click", () => {//Agrega un event listener que ejecutará la función cuando se haga clic en el botón "Buscar".
    ...
    const llamada = fetch(`https://api.giphy.com/v1/gifs/search?api_key=${apikey}&q=${term}&limit=9&offset=0&rating=g&lang=es&bundle =messaging_non_clips`);//Hace una llamada a la API de Giphy utilizando fetch.api_key=${apikey}: La clave para autenticar la solicitud.q=${term}: El término de búsqueda (lo que el usuario escribió).limit=9: Limita el número de GIFs devueltos a 9.offset=0: Empieza desde el primer resultado.rating=g: Filtra resultados aptos para todo público.lang=es: Busca GIFs relacionados con palabras en español.
